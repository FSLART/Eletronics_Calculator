    ...
    try:
        ...
        if filter_type == "Low Pass" and order == "First Order":
            ...
            label_result_filter.config(
                text=f"Cutoff Frequency: {cutoff_frequency:.2f} Hz"
            )
            ...
            if resistance is None or inductance is None or capacitance is None:
                raise ValueError(
                    "Please enter resistance, inductance, and capacitance."
                )
...
def plot_graph_filter():
    try:
        ...
        if filter_type == "Low Pass" and order == "First Order":
            resistance = float(entry_resistance.get())
            capacitance = float(entry_capacitance.get())
            transfer_function = 1 / np.sqrt(
                1 + (2 * np.pi * frequency_range * resistance * capacitance) ** 2
            )
            plt.figure(figsize=(8, 5))
            plt.semilogx(
                frequency_range, 20 * np.log10(transfer_function), label="Low Pass"
            )
        elif filter_type == "High Pass" and order == "First Order":
            resistance = float(entry_resistance.get())
            capacitance = float(entry_capacitance.get())
            transfer_function = (
                2 * np.pi * frequency_range * resistance * capacitance
            ) / np.sqrt(
                1 + (2 * np.pi * frequency_range * resistance * capacitance) ** 2
            )
            plt.figure(figsize=(8, 5))
            plt.semilogx(
                frequency_range, 20 * np.log10(transfer_function), label="High Pass"
            )
        elif filter_type == "Band Pass" and order == "Second Order":
            resistance = float(entry_resistance.get())
            inductance = float(entry_inductance.get())
            ...
            quality_factor = resistance / (2 * np.pi * inductance)
            transfer_function = (frequency_range / omega_0) / np.sqrt(
                (1 - (frequency_range / omega_0) ** 2) ** 2
                + (frequency_range / (omega_0 * quality_factor)) ** 2
            )
            plt.figure(figsize=(8, 5))
            plt.semilogx(
                frequency_range, 20 * np.log10(transfer_function), label="Band Pass"
            )
        elif filter_type == "Pi Filter":
            inductance = float(entry_inductance.get())
            capacitance = float(entry_capacitance.get())
            cutoff_frequency = 1 / (np.pi * np.sqrt(inductance * capacitance))
            transfer_function = 1 / np.sqrt(
                1 + (frequency_range / cutoff_frequency) ** 2
            )
            plt.figure(figsize=(8, 5))
            plt.semilogx(
                frequency_range, 20 * np.log10(transfer_function), label="Pi Filter"
            )
        else:
...
def create_filter_window():
    ...
    frame_filter = tk.Toplevel()
    frame_filter.title("Filter Calculator")

    tk.Label(frame_filter, text="Filter Type:").grid(row=0, column=0, padx=10, pady=5)
    global combo_filter_type
    combo_filter_type = ttk.Combobox(
        frame_filter, values=["Low Pass", "High Pass", "Band Pass", "Pi Filter"]
    )
    ...
    tk.Label(frame_filter, text="Resistance (Ω):").grid(
    ...
    tk.Label(frame_filter, text="Capacitance (F):").grid(
    ...
    tk.Label(frame_filter, text="Inductance (H):").grid(
    ...
    tk.Label(frame_filter, text="Cutout Frequency (Hz):").grid(
    ...
    btn_graph_filter = tk.Button(
        frame_filter, text="Plot Graph", command=plot_graph_filter
    )
    btn_graph_filter.grid(row=7, column=0, columnspan=2, pady=10)

    btn_clear_filter = tk.Button(
        frame_filter, text="Clear", command=clear_fields_filter
    )
