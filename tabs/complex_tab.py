...
def create_complex_tab(notebook):
    ...
    combo_formula_complex.grid(row=1, column=2, padx=10, pady=5)
    combo_formula_complex.bind("<<ComboboxSelected>>", update_complex_fields_based_on_selection)
    combo_formula_complex.current(0)

    tk.Button(tab, text="Calcular",
              command=calculate_complex_to_phasor,
              bg="green",
              fg="white"
              ).grid(row=5, column=1, padx=10, pady=5)
    tk.Button(tab, text="Limpar",
              command=clear_complex_fields,
              bg="blue",
              fg="white"
              ).grid(row=5, column=2, padx=10, pady=5)

    label_result_complex = tk.Label(tab, text="")
    label_result_complex.grid(row=3, column=2, columnspan=2, padx=10, pady=5)
